          ...
          <aside className={`info-panel${openPanel === 'ops' ? ' info-panel--ops' : ''}${openPanel === 'video' && !presentationMode ? ' info-panel--consumer-video' : ''}`} ref={panelRef} role="dialog" aria-modal="true" aria-labelledby="info-panel-title">
            ...
            {openPanel === 'video' ? <>
              ...
              {presentationMode ? <div className="monitor-snapshot">
                ...
                <p><strong>{presenterMonitor?.checkedAtKst ?? presenterMonitor?.checkedAt ?? '—'}</strong> 마지막 자동 확인 · 다음 예약 {presenterMonitor?.scheduleKst ?? '매일 09:17 KST'} · 채널 {presenterMonitor?.sourceChannels ?? 0}/{presenterMonitor?.registeredChannels ?? 0} · 검색어 {presenterMonitor?.discoveryQueries ?? 0}/{presenterMonitor?.totalDiscoveryQueries ?? 0}</p>
