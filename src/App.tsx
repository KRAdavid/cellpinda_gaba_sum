...
const RESEARCH_SOURCES = [
  ...
  },
  {
    ...
    summary: '자연 유래·발효 GABA를 살핀 14개 위약대조 인체시험을 검토한 체계적 문헌고찰입니다. 일부 연구에서 스트레스와 수면 관련 지표의 긍정적 변화가 관찰되었습니다.',
    boundary: '연구마다 섭취량·기간·측정 방법이 달라 긍정적 결과는 각 연구 조건 안에서 해석해야 합니다. 이 문헌고찰은 특정 제품의 효능을 입증하지 않습니다.',
    url: RESEARCH_URL,
  },
  {
    id: 'SRC-04',
...
const PRESENTER_QUESTIONS = [
  ...
  {
    label: '수면·스트레스',
    answer: '수면과 스트레스는 여러 요인의 영향을 받습니다. 일반 GABA 섭취 연구에서 일부 긍정적인 변화가 관찰되었지만, 연구 대상과 조건에 따라 결과가 달라질 수 있어 연구 조건을 함께 확인해야 합니다.',
  },
  {
    label: '일반 연구',
        ...
        {openPanel ? <div className="info-layer" role="presentation" onMouseDown={event => {if (event.target === event.currentTarget) closePanel();}}>
          <aside className={`info-panel${openPanel === 'ops' ? ' info-panel--ops' : ''}${openPanel === 'video' && !presentationMode ? ' info-panel--consumer-video' : ''}`} ref={panelRef} role="dialog" aria-modal="true" aria-labelledby="info-panel-title">
            ...
            {openPanel === 'research' ? <>
              ...
              <p className="info-panel__evidence">연결된 문헌고찰은 일반 GABA 섭취를 살펴본 14개 위약대조 인체시험을 검토했습니다. 일부 연구에서 스트레스와 수면 관련 지표의 긍정적 변화가 관찰되었으며, 섭취량·기간·측정 방법에 따라 결과가 달라질 수 있어 연구 조건을 함께 확인해야 합니다.</p>
