...
사업자 설명 상황에서는 장면 컨트롤의 `발표 모드`를 사용해 한 장면씩 보여줄 수 있습니다. 첫 화면의 `발표자용 설명 시작`은 01번부터 일반 교육 흐름을 시작하고, 발표 중 `영상 DB`에서 승인·보류·제외 후보와 영상별 요약·인물 소개·감리 사유를 확인할 수 있습니다. 영상 DB 패널에는 자동 모니터의 마지막 확인일·검토 대기 수·우선 감리 수·신규 후보 수·현재 브라우저에 저장된 감리 초안 수가 함께 표시되며, `회의용 요약 복사`로 당일 후보·영상 원문·권위·연구 출처 링크 경고·권위 후보 큐를 회의에 넘기고, 작성된 영상 초안은 `작성 초안 전체 복사`로 한 번에 넘길 수 있습니다. `미완료 목록 복사`는 아직 원문·자막·화자·권리·주장 범위를 모두 확인하지 않은 후보와 다음 감리 행동을 회의용 텍스트로 묶습니다. `권위 후보 확인` 큐에는 검색어·제목 기반의 발견 신호가 있는 후보만 따로 모읍니다. 이 큐는 의사·과학자 자격이나 과학적 타당성을 승인한 목록이 아니며, 자격·실제 화자·원문·자막·권리 확인 전에는 고객에게 권위 자료로 소개하지 않습니다. 상세 후보 원문은 감리 보드 링크에서 확인합니다. 선택 영상의 `이 영상 감리 기록 초안`에는 `0/5 확인`처럼 원문·자막·화자·권리·주장 범위의 진행 상태가 먼저 보이며, 담당자·역할·결정 초안·타임코드·확인한 발언·자막 발췌·5개 확인 체크·팀 메모를 남기고 회의용 텍스트로 복사할 수 있습니다. `감리 진행 필요` 필터를 사용하면 다섯 항목을 모두 확인하지 않은 후보만 모아 먼저 처리할 수 있습니다. 입력은 브라우저에만 저장되고 공개 판정은 바뀌지 않습니다. `운영 보드`에서는 사람 역할 배정·과학 출처 검토·영상 공개 판정·A/B/C 사용성 검증의 담당·다음 행동·종료 조건을 확인할 수 있고, `팀 업무 배정 초안 만들기`에서 주 담당·백업·첫 회의 일시를 입력해 회의용 초안을 복사할 수 있습니다. 이 초안은 현재 브라우저에만 저장되며 공식 `0/7` 역할 지표나 공개 승인 상태를 바꾸지 않습니다. 영상 후보가 많을 때는 `검토 필요` 상태 필터나 제목·채널·화자 검색을 사용합니다. 05번 장면에서는 GABA가 감마아미노부티르산을 줄여 부르는 성분 이름이라는 점을 먼저 설명합니다. `Escape`로 발표 모드를 종료하고, 연구·영상 상세는 현재 화면의 패널에서 먼저 설명합니다. `현재 장면 링크 공유`는 모바일 기본 공유 메뉴를 우선 사용하고, 지원하지 않는 환경에서는 링크를 복사합니다.

발표 중 질문이 나오면 `자주 묻는 질문에 답하기`를 열어 GABA 기능·수면·일반 연구·영상 사용에 대한 승인 문장을 확인합니다. 필요하면 각 답변의 `답변 복사`로 전달할 수 있으며, 정보 패널에서는 `이 장면 고객용 링크 복사`로 발표자 모드가 없는 이어보기 링크를 보낼 수 있습니다. 두 기능 모두 현재 장면 흐름을 벗어나지 않으며 소비자 화면에는 표시하지 않습니다.
