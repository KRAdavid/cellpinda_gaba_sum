...
카드의 연구·제품·후기 버튼은 먼저 페이지 안의 정보 패널을 열어 설명 흐름을 유지합니다. `다음 카드`가 기본 행동이며, 원문과 스마트스토어 링크는 패널을 읽은 뒤 `외부 자료는 필요할 때만 확인`을 열어 선택하는 보조 경로입니다. 퍼블리싱 TF의 역할, 검토 기준, 공개 전 수용 기준은 [`docs/PUBLISHING_TF.md`](docs/PUBLISHING_TF.md)에 기록합니다.

사업자 설명 상황에서는 카드 컨트롤의 `발표 모드`를 사용해 한 장씩 보여줄 수 있습니다. 고객이 제품을 먼저 물으면 발표 모드의 `제품부터 설명`으로 현재 화면 안에서 07번 제품 카드로 이동할 수 있습니다. `Escape`로 발표 모드를 종료하고, 연구·제품·후기 상세는 현재 화면의 패널에서 먼저 설명합니다. `현재 카드 링크 공유`는 모바일 기본 공유 메뉴를 우선 사용하고, 지원하지 않는 환경에서는 링크를 복사합니다.
