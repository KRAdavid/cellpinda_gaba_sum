...
소비자 화면의 핵심 흐름은 이제 접속 즉시 첫 메시지가 보이고, 릴스·숏츠처럼 카드 하나가 모바일 viewport를 채우는 세로 피드입니다. 상단에는 현재 위치와 다음 카드 화살표만 두고, 우측 세로 인디케이터로 전체 흐름을 확인합니다. 카드의 연구·영상 버튼은 먼저 페이지 안의 정보 패널을 열어 설명 흐름을 유지하며, `다음 카드`가 기본 행동입니다. 원문 링크는 패널을 읽은 뒤 `외부 자료는 필요할 때만 확인`을 열어 선택하는 보조 경로입니다. 교육 페이지의 소개·범위 안내는 피드를 본 뒤 이어지고, 소비자 화면의 공유·발표 도구는 `더 보기` 안에 접혀 있습니다. 사업자 발표 모드에서는 소개 화면·카드형 발표 화면·영상 DB·운영 보드를 별도로 확인할 수 있습니다. 퍼블리싱 TF의 역할, 검토 기준, 공개 전 수용 기준은 [`docs/PUBLISHING_TF.md`](docs/PUBLISHING_TF.md)에 기록합니다.
