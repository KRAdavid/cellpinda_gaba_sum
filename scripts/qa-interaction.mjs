...
try {
  ...
  await wait(700);
  const videoShowcase = await evaluate('({section:!!document.querySelector("#video-showcase"),title:document.querySelector("#video-showcase-title")?.innerText||"",items:document.querySelectorAll(".video-showcase__item").length,summaries:document.querySelectorAll(".video-showcase__copy").length,links:[...document.querySelectorAll(".video-showcase__actions a")].map(link=>link.getAttribute("href")||""),preview:document.querySelectorAll(".video-showcase__media img, .video-showcase__source-mark").length,previewImageLoaded:[...document.querySelectorAll(".video-showcase__media img")].every(image => image.complete && image.naturalWidth > 0),mediaButtons:document.querySelectorAll(".video-showcase__media").length,body:document.querySelector("#video-showcase")?.innerText||""})');
  assert('consumer video summaries live in a separate showcase section', videoShowcase.section && videoShowcase.title.includes('원문으로 확인') && videoShowcase.items === 7 && videoShowcase.summaries === 7 && videoShowcase.links.some(link => link.includes('youtube.com/shorts/RLAU1VWGsaI')) && videoShowcase.links.some(link => link.includes('youtube.com/shorts/4xGSHxkMYew')) && !videoShowcase.links.some(link => link.includes('dnalc.cshl.edu') || link.includes('videocast.nih.gov')) && videoShowcase.preview === 7 && videoShowcase.previewImageLoaded && videoShowcase.mediaButtons === 7 && videoShowcase.body.includes('무엇을 어떻게 소개했나') && videoShowcase.body.includes('인물 소개') && videoShowcase.body.includes('오늘 공유된 검토 후보') && !videoShowcase.body.includes('권위 영상 DB'), JSON.stringify(videoShowcase));
  await evaluate('document.querySelector(".video-showcase__media")?.click()');
  await waitForText('#info-panel-title', 'GABA 영상 DB 검토');
  const publicVideo = await evaluate('({items:document.querySelectorAll(".video-db-item").length,detail:document.querySelector(".video-db-detail")?.innerText||"",external:document.querySelector(".info-panel__external")?.getAttribute("href")||"",channel:document.querySelector(".video-db-detail__meta a")?.getAttribute("href")||"",preview:!!document.querySelector(".video-db-preview") && (!!document.querySelector(".video-db-preview img") || !!document.querySelector(".video-db-preview__source-mark")),body:document.querySelector(".info-panel")?.innerText||""})');
  assert('consumer video detail remains available from the separate showcase', publicVideo.items === 7 && publicVideo.detail.includes('인물 소개') && publicVideo.detail.includes('확인 기반') && publicVideo.detail.includes('권위') && publicVideo.preview && publicVideo.external.includes('youtube.com/shorts/') && publicVideo.channel.includes('youtube.com/@bm_k_clinic') && !publicVideo.body.includes('dnalc.cshl.edu') && !publicVideo.body.includes('videocast.nih.gov'), JSON.stringify(publicVideo));
  ...
  await waitForText('#info-panel-title', 'GABA 영상 DB 검토');
  const presenterDb = await evaluate('({items:document.querySelectorAll(".video-db-item").length,hasHold:document.querySelector(".video-db-list")?.innerText.includes("검토 보류")||false,detail:document.querySelector(".video-db-detail")?.innerText||"",body:document.querySelector(".video-db-list")?.innerText||"",search:!!document.querySelector(".video-db-search input"),filters:document.querySelectorAll(".video-db-filters button").length})');
  assert('presenter video DB keeps candidate review separate from public curation', presenterDb.items === 9 && presenterDb.hasHold && !presenterDb.detail && !presenterDb.body?.includes('Molecular regulation'), JSON.stringify(presenterDb));
