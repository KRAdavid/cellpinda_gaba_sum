...
try {
  ...
  const researchPanel = await evaluate('({title:document.querySelector("#info-panel-title")?.innerText||"",source:document.querySelector(".info-panel__source")?.innerText||"",verification:document.querySelector(".info-panel__verification")?.innerText||"",evidence:document.querySelector(".info-panel__evidence")?.innerText||"",external:document.querySelector(".info-panel__external")?.getAttribute("href")||"",url:location.href})');
   assert('research opens in an in-page panel', researchPanel.title.includes('일반 GABA 연구') && researchPanel.source.includes('PMID 33041752') && researchPanel.verification.includes('결과를 볼 때 함께 확인할 것') && researchPanel.verification.includes('누구를 대상으로') && !researchPanel.verification.includes('AI 사전 확인') && !researchPanel.verification.includes('SCIENCE·MEDICAL') && researchPanel.evidence.includes('스트레스·수면 관련 지표가 좋아지는 변화가 확인') && researchPanel.evidence.includes('참여자·섭취량·기간·비교 방식') && !researchPanel.evidence.includes('근거는 제한적') && researchPanel.external.includes('pubmed.ncbi.nlm.nih.gov') && !researchPanel.url.includes('pubmed'), JSON.stringify(researchPanel));
  ...
  const researchSources = await evaluate('({summary:document.querySelector(".info-panel__research-sources summary")?.innerText||"",items:document.querySelectorAll(".research-source-list article").length,body:document.querySelector(".research-source-list")?.innerText||""})');
   assert('research panel keeps multiple evidence sources in-page', researchSources.summary.includes('참고한 연구·자료 보기') && researchSources.items === 5 && researchSources.body.includes('PMID 22203366') && researchSources.body.includes('스트레스·수면 관련 지표가 좋아지는 변화가 확인') && !researchSources.body.includes('근거는 제한적'), JSON.stringify(researchSources));
