...
try {
  ...
  assert('consumer video detail remains available from the separate showcase', publicVideo.items === 7 && publicVideo.detail.includes('인물 소개') && publicVideo.detail.includes('요약 근거') && publicVideo.detail.includes('권위') && publicVideo.detail.includes('화자·소속 확인 출처') && publicVideo.preview && publicVideo.embed && publicVideo.localUrl && publicVideo.external.includes('youtube.com/shorts/') && publicVideo.channels.some(link => link.includes('youtube.com/channel/UC-eyEDlbCD_8epmh-qCJ9oA/about')) && publicVideo.channels.some(link => link.includes('bmkclinic.com')) && !publicVideo.body.includes('dnalc.cshl.edu') && !publicVideo.body.includes('videocast.nih.gov') && !publicVideo.body.includes('수면제') && !publicVideo.body.includes('영양제') && !publicVideo.body.includes('보충제'), JSON.stringify(publicVideo));
