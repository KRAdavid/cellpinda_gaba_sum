...
try {
  ...
  assert('consumer video summaries use a one-at-a-time review flow', videoShowcase.section && videoShowcase.title.includes('원문으로 확인') && videoShowcase.items === 7 && videoShowcase.summaries === 1 && videoShowcase.links.length === 0 && videoShowcase.actionButtons === 1 && videoShowcase.preview === 1 && videoShowcase.previewImageLoaded && videoShowcase.mediaButtons === 1 && videoShowcase.body.includes('오늘의 검토 흐름') && videoShowcase.body.includes('무엇을 어떻게 소개했나') && videoShowcase.body.includes('인물 소개') && videoShowcase.body.includes('권위 확인') && videoShowcase.body.includes('근거') && videoShowcase.body.includes('요약 근거') && videoShowcase.body.includes('사용') && videoShowcase.body.includes('오늘 공유된 검토 후보') && videoShowcase.body.includes('원문 링크는 상세 패널에서 선택') && videoShowcase.body.includes('검토 보류') && videoShowcase.body.includes('제한 사용') && !videoShowcase.body.includes('권위 영상 DB') && !videoShowcase.body.includes('수면제') && !videoShowcase.body.includes('영양제') && !videoShowcase.body.includes('보충제'), JSON.stringify(videoShowcase));
  ...
  assert('presenter video DB shows daily monitoring snapshot', monitorSnapshot.summary.includes('마지막 자동 확인') && monitorSnapshot.summary.includes('검토 대기') && monitorSnapshot.summary.includes('권위·연구 출처 링크') && monitorSnapshot.summary.includes('등록 YouTube 자막 트랙') && monitorSnapshot.summary.includes('등록 YouTube 자막 본문') && monitorSnapshot.summary.includes('본문 경고') && monitorSnapshot.summary.includes('오늘 먼저 검토할 후보') && monitorSnapshot.summary.includes('권위 후보 확인') && monitorSnapshot.summary.includes('최근 감리 추이') && monitorSnapshot.summary.includes('첫 담당') && monitorSnapshot.summary.includes('다음 행동') && monitorSnapshot.summary.includes('오늘 리뷰 세션') && monitorSnapshot.summary.includes('자막 감사 기록') && monitorSnapshot.queue === 5 && monitorSnapshot.history >= 1 && monitorSnapshot.authorityQueue >= 3 && monitorSnapshot.links === 4, JSON.stringify(monitorSnapshot));
  ...
  await wait(180);
  const savedReviewDraft = await evaluate('(() => { const raw=localStorage.getItem("cellpinda-gaba-video-review-draft-v1"); const saved=raw ? JSON.parse(raw) : {}; const draft=saved["SHORT-02"]||{}; return {reviewer:draft.reviewer||"",timestamps:draft.timestamps||"",notes:draft.notes||"",checked:!!draft.sourceChecked,decision:draft.decision||""}; })()');
  assert('presenter review draft persists locally without changing video status', savedReviewDraft.reviewer === 'VIDEO 담당' && savedReviewDraft.timestamps.includes('00:12') && savedReviewDraft.notes.includes('일반 GABA') && savedReviewDraft.checked && savedReviewDraft.decision === 'UNDECIDED', JSON.stringify(savedReviewDraft));
