...
try {
  ...
  assert('consumer video summaries use a one-at-a-time review flow', videoShowcase.section && videoShowcase.title.includes('원문으로 확인') && videoShowcase.items === 7 && videoShowcase.summaries === 1 && videoShowcase.links.length === 0 && videoShowcase.actionButtons === 1 && videoShowcase.preview === 1 && videoShowcase.previewImageLoaded && videoShowcase.mediaButtons === 1 && videoShowcase.body.includes('오늘의 검토 흐름') && videoShowcase.body.includes('무엇을 어떻게 소개했나') && videoShowcase.body.includes('인물 소개') && videoShowcase.body.includes('권위 확인') && videoShowcase.body.includes('근거') && videoShowcase.body.includes('사용') && videoShowcase.body.includes('오늘 공유된 검토 후보') && videoShowcase.body.includes('원문 링크는 상세 패널에서 선택') && videoShowcase.body.includes('검토 보류') && videoShowcase.body.includes('제한 사용') && !videoShowcase.body.includes('권위 영상 DB'), JSON.stringify(videoShowcase));
  ...
  assert('consumer can move to the next video without leaving the showcase', secondShowcaseVideo.active.includes('02') && secondShowcaseVideo.meta.includes('SHORT-04') && secondShowcaseVideo.title.includes('불면증'), JSON.stringify(secondShowcaseVideo));
  ...
  const monitorSnapshot = await evaluate('({summary:document.querySelector(".monitor-snapshot")?.innerText||"",links:document.querySelectorAll(".monitor-snapshot a").length,queue:document.querySelectorAll(".monitor-snapshot__queue li details").length,openQueue:document.querySelectorAll(".monitor-snapshot__queue li details[open]").length})');
  assert('presenter video DB shows daily monitoring snapshot', monitorSnapshot.summary.includes('마지막 자동 확인') && monitorSnapshot.summary.includes('검토 대기') && monitorSnapshot.summary.includes('등록 YouTube 자막 트랙') && monitorSnapshot.summary.includes('등록 YouTube 자막 본문') && monitorSnapshot.summary.includes('본문 경고') && monitorSnapshot.summary.includes('오늘 먼저 검토할 후보') && monitorSnapshot.summary.includes('첫 담당') && monitorSnapshot.summary.includes('다음 행동') && monitorSnapshot.summary.includes('오늘 리뷰 세션') && monitorSnapshot.queue === 5 && monitorSnapshot.links === 3, JSON.stringify(monitorSnapshot));
