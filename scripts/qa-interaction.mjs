...
try {
  ...
  const readerLayout = await evaluate('(() => { const story=document.querySelector("#story"), stream=document.querySelector(".story-reader-stream"), scene=document.querySelector(".story-reader-scene"), index=document.querySelector(".story-reader-index"), mobileProgress=document.querySelector(".story-reader-index__mobile-progress"), storyStyle=getComputedStyle(story), streamStyle=getComputedStyle(stream), sceneStyle=getComputedStyle(scene); return {storyHeight:story?.getBoundingClientRect().height||0,streamHeight:stream?.getBoundingClientRect().height||0,sceneWidth:scene?.getBoundingClientRect().width||0,contentWidth:document.documentElement.clientWidth,index:!!index,mobileProgress:!!mobileProgress,scenes:document.querySelectorAll(".story-reader-scene").length,layout:streamStyle.display,storyBackground:storyStyle.backgroundColor,sceneBorder:sceneStyle.borderBottomWidth}; })()');
  assert('consumer reading flow uses open scenes instead of stacked cards', readerLayout.scenes === 8 && readerLayout.index && readerLayout.mobileProgress && readerLayout.streamHeight > viewportHeight && readerLayout.sceneWidth < readerLayout.contentWidth && readerLayout.storyBackground !== 'rgb(22, 59, 44)', JSON.stringify(readerLayout));
