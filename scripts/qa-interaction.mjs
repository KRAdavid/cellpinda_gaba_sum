...
try {
  ...
  assert('presenter video DB keeps candidate review separate from public curation', presenterDb.items === 11 && presenterDb.hasHold && !presenterDb.detail && !presenterDb.body?.includes('Molecular regulation') && presenterDb.panel.includes('국내 공개 승인 0건') && presenterDb.panel.includes('DB 승인 이력 2건') && presenterDb.panel.includes('감리 초안 0건'), JSON.stringify(presenterDb));
  ...
  assert('presenter video DB offers a three-item priority review batch from the domestic register', reviewBatch.count === 3 && reviewBatch.buttons === 3 && reviewBatch.ids.every(id => !id.startsWith('AUTH-') && id !== 'SHORT-01') && reviewBatch.text.includes('국내 등록 영상 11건') && reviewBatch.text.includes('5개 확인 항목') && reviewBatch.text.includes('사람의 공개 판단'), JSON.stringify(reviewBatch));
  ...
  const monitorSnapshot = await evaluate('({summary:document.querySelector(".monitor-snapshot")?.innerText||"",links:document.querySelectorAll(".monitor-snapshot a").length,queue:document.querySelectorAll(".monitor-snapshot__queue li details").length,authorityQueue:document.querySelectorAll(".monitor-snapshot__authority-queue li details").length,productQueue:document.querySelectorAll(".monitor-snapshot__product-queue li details").length,history:document.querySelectorAll(".monitor-snapshot__history li").length,openQueue:document.querySelectorAll(".monitor-snapshot__queue li details[open]").length})');
  assert('presenter video DB shows daily monitoring snapshot', monitorSnapshot.summary.includes('마지막 자동 확인') && monitorSnapshot.summary.includes('검토 대기') && monitorSnapshot.summary.includes('권위·연구 출처 링크') && monitorSnapshot.summary.includes('등록 YouTube 자막 트랙') && monitorSnapshot.summary.includes('등록 YouTube 자막 본문') && monitorSnapshot.summary.includes('본문 경고') && monitorSnapshot.summary.includes('HTTP 429 접근 제한') && monitorSnapshot.summary.includes('오늘 먼저 검토할 후보') && monitorSnapshot.summary.includes('권위 후보 확인 전') && monitorSnapshot.summary.includes('제품·브랜드 신호') && monitorSnapshot.summary.includes('일반 공개 큐 제외') && !monitorSnapshot.summary.includes('셀핀다') && monitorSnapshot.summary.includes('전문가 표현 감지') && monitorSnapshot.summary.includes('최근 감리 추이') && monitorSnapshot.summary.includes('첫 담당') && monitorSnapshot.summary.includes('다음 행동') && monitorSnapshot.summary.includes('오늘 리뷰 세션') && monitorSnapshot.summary.includes('메타데이터 감사 기록') && monitorSnapshot.summary.includes('자막 감사 기록') && monitorSnapshot.queue === 5 && monitorSnapshot.authorityQueue >= 3 && monitorSnapshot.productQueue === 5 && monitorSnapshot.history >= 1 && monitorSnapshot.links === 5, JSON.stringify(monitorSnapshot));
  ...
  assert('presenter video DB filters review status before selection', presenterDb.search && presenterDb.filters === 8 && holdFilter.items === 7 && holdFilter.active === 'true', JSON.stringify({presenterDb,holdFilter}));
  ...
  assert('presenter video DB can isolate unfinished audit drafts', incompleteFilter.items === 11 && incompleteFilter.active.includes('감리 진행 필요'), JSON.stringify(incompleteFilter));
  ...
  assert('presenter video DB can prioritize candidates with person evidence', profileFilter.items === 7 && profileFilter.active.includes('인물 출처 있음') && profileFilter.label.includes('인물 출처'), JSON.stringify(profileFilter));
  await evaluate('document.querySelector(".video-db-filters button:first-child")?.click()');
  await wait(120);
  ...
  assert('presenter review draft summary exposes batch copy, DB export, handoff, and gated publication request controls', reviewSummary.count.includes('감리 초안 1건') && reviewSummary.count.includes('미완료 11건') && reviewSummary.buttons === 5 && reviewSummary.disabled === false && reviewSummary.exportButton && reviewSummary.handoffExportButton && reviewSummary.handoffImportButton && reviewSummary.releaseButton && reviewSummary.releaseDisabled, JSON.stringify(reviewSummary));
