...
try {
  ...
  const reviewDraft = await evaluate('({open:document.querySelector(".video-review-draft")?.open||false,fields:document.querySelectorAll(".video-review-draft__fields [data-review-field]").length,checks:document.querySelectorAll(".video-review-draft__checks input[type=checkbox]").length,notes:document.querySelector(".video-review-draft__notes textarea")?.getAttribute("placeholder")||"",boundary:document.querySelector(".video-review-draft__note")?.innerText||""})');
  assert('presenter video DB offers a local per-video review draft', reviewDraft.open && reviewDraft.fields === 4 && reviewDraft.checks === 5 && reviewDraft.notes.includes('확인한 발언') && reviewDraft.boundary.includes('공개 승인'), JSON.stringify(reviewDraft));
