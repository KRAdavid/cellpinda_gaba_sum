...
try {
  ...
  assert('presenter video DB offers a three-item priority review batch from the domestic register', reviewBatch.count === 3 && reviewBatch.buttons === 3 && reviewBatch.ids.every(id => !id.startsWith('AUTH-') && id !== 'SHORT-01') && reviewBatch.text.includes('국내 등록 영상 10건') && reviewBatch.text.includes('5개 확인 항목') && reviewBatch.text.includes('사람의 공개 판단'), JSON.stringify(reviewBatch));
