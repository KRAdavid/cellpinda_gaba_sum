...
try {
  ...
  const initial = await evaluate('(() => { const scenes=[...document.querySelectorAll(".story-reader-scene")]; const body=document.body.innerText; const gesture=document.querySelector(".consumer-reel__gesture"); return {title:document.title,width:innerWidth,height:innerHeight,clientWidth:document.documentElement.clientWidth,docWidth:document.documentElement.scrollWidth,scenes:scenes.length,legacyCards:document.querySelectorAll(".story-card").length,progress:(document.querySelector(".consumer-reel__counter strong")?.innerText||"")+" / 08",storyTop:document.querySelector("#story")?.getBoundingClientRect().top||0,heading:!!document.querySelector(".consumer-reel__topline"),gestureVisible:gesture?.classList.contains("is-visible") ?? false,gestureText:gesture?.innerText || "",publicIntro:document.querySelectorAll(".intro").length,presenterStart:/(발표자용|발표 모드)/.test(body),body}; })()');
  ...
  assert('consumer page omits internal review wording', !initial.body.includes('검토 후보') && !initial.body.includes('연구 읽는 기준') && !initial.body.includes('근거는 조건까지'));
  ...
  assert('first scene exposes a clear swipe cue', initial.gestureVisible && initial.gestureText.includes('위로 넘겨 다음 장면'), JSON.stringify(initial));
  ...
  assert('reader opening presents one focused message with a reel action', readerOpening.headline.includes('집중이 흐트러지고') && readerOpening.visual && readerOpening.next && readerOpening.progress && !readerOpening.hasCardCopy, JSON.stringify(readerOpening));
  ...
  assert('consumer reading flow uses one open page instead of stacked cards', readerLayout.scenes === 8 && readerLayout.visibleScenes === 1 && readerLayout.hiddenScenes === 7 && readerLayout.progress && readerLayout.streamHeight > 0 && readerLayout.sceneWidth < readerLayout.contentWidth && readerLayout.stageId === 'story-scene-hook' && readerLayout.storyBackground === 'rgb(251, 252, 248)' && readerLayout.sceneBorder === '0px', JSON.stringify(readerLayout));
  ...
  assert('reading flow exposes the next message action', nextBar.visible && nextBar.button && !nextBar.link && nextBar.text.includes('다음 장면') && nextBar.text.includes('02 · 여유가 생기는 날') && nextBar.mobileButton && nextBar.mobileText.includes('여유가 생기는 날'), JSON.stringify(nextBar));

  await evaluate('document.querySelector(".consumer-reel__next-button")?.click()');
  await waitForProgress('02 / 08');
  await wait(900);
  const paged = await evaluate('(() => { const scenes=[...document.querySelectorAll(".story-reader-scene")]; const gesture=document.querySelector(".consumer-reel__gesture"); return {progress:(document.querySelector(".consumer-reel__counter strong")?.innerText||"")+" / 08",activeTitle:document.querySelector(".story-reader-scene.is-active h2")?.innerText||"",visibleScenes:scenes.filter(item => getComputedStyle(item).display !== "none").length,stageTop:document.querySelector("#story-scene-hook")?.getBoundingClientRect().top ?? 999,gestureVisible:gesture?.classList.contains("is-visible") ?? false,gestureHidden:gesture?.getAttribute("aria-hidden") === "true"}; })()');
  assert('page-turn action advances one focused message', paged.progress === '02 / 08' && paged.activeTitle.includes('잠을 충분히 잔 날') && paged.visibleScenes === 1 && Math.abs(paged.stageTop) < 120, JSON.stringify(paged));
  assert('swipe cue clears after the first page turn', !paged.gestureVisible && paged.gestureHidden, JSON.stringify(paged));
  const nextBarAfterPageTurn = await evaluate('document.querySelector(".consumer-reel__next")?.innerText||""');
  assert('page-turn flow previews the following message', nextBarAfterPageTurn.includes('다음 장면') && nextBarAfterPageTurn.includes('03 · 머리가 계속 바쁜 날'), nextBarAfterPageTurn);
  ...
  const researchPanel = await evaluate('({title:document.querySelector("#info-panel-title")?.innerText||"",source:document.querySelector(".info-panel__source")?.innerText||"",verification:document.querySelector(".info-panel__verification")?.innerText||"",evidence:document.querySelector(".info-panel__evidence")?.innerText||"",external:document.querySelector(".info-panel__external")?.getAttribute("href")||"",url:location.href})');
   assert('research opens in an in-page panel', researchPanel.title.includes('일반 GABA 연구') && researchPanel.source.includes('PMID 33041752') && researchPanel.verification.includes('결과를 볼 때 함께 확인할 것') && researchPanel.verification.includes('누구를 대상으로') && !researchPanel.verification.includes('AI 사전 확인') && !researchPanel.verification.includes('SCIENCE·MEDICAL') && researchPanel.evidence.includes('스트레스·수면 관련 지표가 개선된 결과가 보고') && researchPanel.evidence.includes('참여자·섭취량·기간·비교 방식') && !researchPanel.evidence.includes('근거는 제한적') && researchPanel.external.includes('pubmed.ncbi.nlm.nih.gov') && !researchPanel.url.includes('pubmed'), JSON.stringify(researchPanel));
  ...
  const researchSources = await evaluate('({summary:document.querySelector(".info-panel__research-sources summary")?.innerText||"",items:document.querySelectorAll(".research-source-list article").length,body:document.querySelector(".research-source-list")?.innerText||""})');
   assert('research panel keeps multiple evidence sources in-page', researchSources.summary.includes('참고한 연구·자료 보기') && researchSources.items === 5 && researchSources.body.includes('PMID 22203366') && researchSources.body.includes('스트레스·수면 관련 지표가 개선된 결과가 보고') && !researchSources.body.includes('근거는 제한적'), JSON.stringify(researchSources));
  ...
  const footerResearch = await evaluate('({title:document.querySelector("#info-panel-title")?.innerText||"",url:location.href,external:document.querySelector(".info-panel__external")?.getAttribute("href")||"",footerButton:document.querySelector(".site-footer__research-button")?.innerText||""})');
   assert('footer research action keeps the reader in-page before external source choice', footerResearch.title.includes('일반 GABA 연구') && !footerResearch.url.includes('pubmed') && footerResearch.external.includes('pubmed.ncbi.nlm.nih.gov') && footerResearch.footerButton.includes('연구 출처 보기'), JSON.stringify(footerResearch));
  ...
  assert('consumer video summaries use an in-page one-at-a-time reel flow', videoShowcase.section && videoShowcase.title.includes('원문으로 확인') && videoShowcase.items === 7 && videoShowcase.summaries === 1 && videoShowcase.links.length === 0 && videoShowcase.actionButtons === 2 && videoShowcase.reelControls === 2 && videoShowcase.nextReel.includes('다음 영상') && !videoShowcase.player && videoShowcase.poster && videoShowcase.playerHeading.includes('GABA 설명 영상') && videoShowcase.playerHeading.includes('페이지 안에서 재생') && videoShowcase.playerNote.includes('영상 전체 내용은 YouTube 원문') && videoShowcase.candidateNotes === 1 && videoShowcase.body.includes('영상 목록') && videoShowcase.body.includes('영상 요약') && videoShowcase.body.includes('출연자와 채널') && videoShowcase.body.includes('원문 확인 전 참고 영상') && videoShowcase.body.includes('영상 정보 보기') && videoShowcase.body.includes('영상 요약은 제목과 설명') && videoShowcase.body.includes('전체 내용은 YouTube 원문에서 확인하세요') && videoShowcase.body.includes('이 영상 링크 공유') && videoShowcase.body.includes('위로 밀어 다음 영상') && !videoShowcase.body.includes('영상 읽는 기준') && !videoShowcase.body.includes('업데이트 기준') && !videoShowcase.body.includes('감리') && !videoShowcase.body.includes('권위 확인') && !videoShowcase.body.includes('요약 근거') && !videoShowcase.body.includes('사람 감리') && !videoShowcase.body.includes('공개 승인') && !videoShowcase.body.includes('원출처·재게시') && !videoShowcase.body.includes('검토 후보') && !videoShowcase.body.includes('수면제') && !videoShowcase.body.includes('영양제') && !videoShowcase.body.includes('보충제'), JSON.stringify(videoShowcase));
  ...
  await waitForText('.video-showcase__copy h3', 'GABA와 뇌 신호를 알아보는 영상');
  const swipeShowcaseVideo = await evaluate('({active:document.querySelector(".video-showcase__index-button.is-active")?.innerText||"",progress:document.querySelector(".video-showcase__reel-controls span strong")?.innerText||""})');
  assert('consumer video reel advances with a vertical swipe', swipeShowcaseVideo.active.includes('02') && swipeShowcaseVideo.progress.includes('02 / 07'), JSON.stringify(swipeShowcaseVideo));
  await evaluate('document.querySelectorAll(".video-showcase__index-button")[0]?.click()');
  await waitForText('.video-showcase__copy h3', 'GABA의 역할을 소개하는 영상');
  ...
  assert('consumer video detail remains available from the separate showcase', publicVideo.items === 7 && publicVideo.detail.includes('영상 요약') && publicVideo.detail.includes('출연자와 채널') && publicVideo.detail.includes('한 문장으로 정리하면') && publicVideo.detail.includes('더 알아보기') && !publicVideo.detail.includes('요약 근거') && !publicVideo.detail.includes('권위') && !publicVideo.detail.includes('확인일') && !publicVideo.detail.includes('사업자 설명 한 문장') && !publicVideo.detail.includes('다음 감리 행동') && !publicVideo.detail.includes('확인할 점') && publicVideo.preview && publicVideo.embed && publicVideo.localUrl && publicVideo.external.includes('youtube.com/shorts/') && publicVideo.channels.length === 0 && publicVideo.panelClass.includes('info-panel--consumer-video') && publicVideo.actionPosition === 'static' && publicVideo.body.includes('GABA 관련 참고 영상') && !publicVideo.body.includes('원문 확인 전') && !publicVideo.body.includes('제한 사용 후보') && !publicVideo.body.includes('dnalc.cshl.edu') && !publicVideo.body.includes('videocast.nih.gov') && !publicVideo.body.includes('수면제') && !publicVideo.body.includes('영양제') && !publicVideo.body.includes('보충제'), JSON.stringify(publicVideo));
  ...
  assert('consumer video detail keeps the speaker summary without internal evidence fields', sleepEvidence.detail.includes('출연자와 채널') && sleepEvidence.detail.includes('SLEEP Dr.') && sleepEvidence.links.length === 0, JSON.stringify(sleepEvidence));
  await evaluate('document.querySelectorAll(".video-db-item__select")[1]?.click()');
  await waitForText('.video-db-detail', '브레인튜브');
  await evaluate('document.querySelector(".info-panel__next")?.click()');
  ...
  assert('presenter keeps one focused teaching scene in an open non-stacked canvas', presenterLayout.label.includes('01') && presenterLayout.title.includes('집중이 흐트러지고') && presenterLayout.railHeight >= 300 && presenterLayout.railOverflow.includes('hidden') && presenterLayout.radius === '0px' && presenterLayout.shadow === 'none' && presenterLayout.frame === '0px', JSON.stringify(presenterLayout));
  ...
  assert('presenter video DB shows daily monitoring snapshot', monitorSnapshot.summary.includes('마지막 실행 기록') && monitorSnapshot.summary.includes('실행 출처') && monitorSnapshot.summary.includes('오늘 신규 후보') && monitorSnapshot.summary.includes('이번 실행') && monitorSnapshot.summary.includes('자동 공개') && monitorSnapshot.summary.includes('검토 대기') && monitorSnapshot.summary.includes('권위·연구 출처 링크') && monitorSnapshot.summary.includes('등록 YouTube 자막 트랙') && monitorSnapshot.summary.includes('등록 YouTube 자막 본문') && monitorSnapshot.summary.includes('본문 경고') && monitorSnapshot.summary.includes('HTTP 429 접근 제한') && monitorSnapshot.summary.includes('오늘 먼저 검토할 후보') && monitorSnapshot.summary.includes('권위 후보 확인 전') && monitorSnapshot.summary.includes('제품·브랜드 신호') && monitorSnapshot.summary.includes('일반 공개 큐 제외') && !monitorSnapshot.summary.includes('셀핀다') && monitorSnapshot.summary.includes('전문가 표현 감지') && monitorSnapshot.summary.includes('최근 감리 추이') && monitorSnapshot.summary.includes('첫 담당') && monitorSnapshot.summary.includes('다음 행동') && monitorSnapshot.summary.includes('오늘 리뷰 세션') && monitorSnapshot.summary.includes('메타데이터 감사 기록') && monitorSnapshot.summary.includes('자막 감사 기록') && monitorSnapshot.reviewLog.includes('팀 리뷰 로그') && monitorSnapshot.queue === 5 && monitorSnapshot.authorityQueue >= 3 && monitorSnapshot.productQueue === 5 && monitorSnapshot.history >= 1 && monitorSnapshot.links >= 8, JSON.stringify(monitorSnapshot));
  ...
  assert('video DB selection shows summary, person introduction, review rule, operator sentence, customer brief preview, and next audit action', selected.detail.includes('무엇을 어떻게 소개했나') && selected.detail.includes('인물 소개') && selected.detail.includes('영상별 감리 규칙') && selected.detail.includes('사업자 설명 한 문장') && selected.detail.includes('고객 설명 3문장 복사') && selected.detail.includes('고객 설명 3문장 미리 보기') && selected.detail.includes('고객 전달용 문장은 미승인 영상을') && selected.detail.includes('원문 확인 전') && selected.detail.includes('다음 감리 행동') && selected.external.includes('youtube.com/shorts/RLAU1VWGsaI'), JSON.stringify(selected));
