...
try {
  ...
  const initialPage = await evaluate('(() => { const cards=[...document.querySelectorAll(".story-card")]; return {title:document.title,width:innerWidth,clientWidth:document.documentElement.clientWidth,docWidth:document.documentElement.scrollWidth,presenterGuidance:!!document.querySelector(".presenter-note"),presenterQuestions:!!document.querySelector(".presenter-questions"),presenterProductShortcut:!!document.querySelector(".story-product-start"),carousel:document.querySelector(".story-rail")?.getAttribute("aria-roledescription")||"",currentCard:document.querySelector(".story-card[aria-current=\\"true\\"]")?.getAttribute("aria-label")||"",hiddenCount:cards.filter(card=>card.getAttribute("aria-hidden")==="true").length,inertCount:cards.filter(card=>card.hasAttribute("inert")).length,activeHidden:document.querySelector(".story-card[aria-current=\\"true\\"]")?.getAttribute("aria-hidden")||""}; })()');
  ...
  assert('consumer carousel hides inactive slides from assistive technology', initialPage.hiddenCount === 10 && initialPage.inertCount === 10 && !initialPage.activeHidden, JSON.stringify(initialPage));

  const introEntry = await evaluate('(() => { const buttons = [...document.querySelectorAll(".intro .text-button")]; const rects = buttons.map(button => { const rect = button.getBoundingClientRect(); return {label:button.innerText, left:rect.left, right:rect.right, top:rect.top, bottom:rect.bottom, width:rect.width, height:rect.height}; }); const visible = rects.filter(rect => rect.bottom > 0 && rect.top < innerHeight && rect.width > 0 && rect.height > 0); const overlap = visible.some((left, index) => visible.slice(index + 1).some(right => left.left < right.right && left.right > right.left && left.top < right.bottom && left.bottom > right.top)); const primary = rects.find(rect => rect.label.includes("전체 카드부터 보기")); return {labels:rects.map(rect=>rect.label),primaryVisible:!!primary && primary.top >= 0 && primary.bottom <= innerHeight && primary.width > 0 && primary.height > 0,visibleCount:visible.length,overlap}; })()');
  assert('intro entry actions stay visible and non-overlapping', introEntry.primaryVisible && !introEntry.overlap && introEntry.labels.some(label => label.includes("사업자용 설명 시작")) && introEntry.labels.some(label => label.includes("제품 정보가 먼저라면")), JSON.stringify(introEntry));
  const introHierarchy = await evaluate('(() => { const primary=document.querySelector(".intro-primary-button"); const secondary=document.querySelector(".intro-secondary-actions"); const primaryStyle=primary ? getComputedStyle(primary) : null; return {primary:!!primary && primary.getBoundingClientRect().height > 0,secondary:!!secondary && secondary.getBoundingClientRect().height > 0,primaryBackground:primaryStyle?.backgroundColor||"",label:document.querySelector(".intro-secondary-label")?.innerText||""}; })()');
  assert('intro makes the full story the primary entry', introHierarchy.primary && introHierarchy.secondary && introHierarchy.primaryBackground !== 'rgba(0, 0, 0, 0)' && introHierarchy.label.includes('사업자 설명·제품 정보'), JSON.stringify(introHierarchy));

  await evaluate('document.getElementById("story")?.scrollIntoView({behavior:"auto"})');
  await wait(200);
  const railBehavior = await evaluate('(() => { const rail = document.querySelector(".story-rail"); const style = rail ? getComputedStyle(rail) : null; const cardStyle = document.querySelector(".story-card") ? getComputedStyle(document.querySelector(".story-card")) : null; return {touchAction:style?.touchAction || "", snap:style?.scrollSnapType || "", overflow:style?.overflowX || "",stop:cardStyle?.scrollSnapStop || ""}; })()');
  if (viewportWidth <= 760) assert('mobile rail declares horizontal touch and snap', railBehavior.touchAction === 'pan-x' && railBehavior.snap.includes('x') && railBehavior.overflow === 'auto' && railBehavior.stop === 'always', JSON.stringify(railBehavior));
  ...
  const presenterSequenceLayout = await evaluate('(() => { const nav=document.querySelector(".story--presentation .story-sequence")?.getBoundingClientRect(); const children=[...document.querySelectorAll(".story--presentation .story-sequence > span")].map(item=>{const rect=item.getBoundingClientRect(); return {top:rect.top,bottom:rect.bottom};}); return {nav:nav ? {top:nav.top,bottom:nav.bottom,height:nav.height} : null,children,within:!!nav && children.every(child=>child.top >= nav.top - 1 && child.bottom <= nav.bottom + 1)}; })()');
  if (viewportWidth <= 760) assert('presenter mobile sequence labels stay inside their row', presenterSequenceLayout.within, JSON.stringify(presenterSequenceLayout));
  const presenterControlLayout = await evaluate('(() => { const buttons = [...document.querySelectorAll(".story--presentation .story-controls button")]; const rects = buttons.map(button => { const rect = button.getBoundingClientRect(); return {label:button.innerText, left:rect.left, right:rect.right, top:rect.top, bottom:rect.bottom, width:rect.width, height:rect.height}; }); const overlap = rects.some((left, index) => rects.slice(index + 1).some(right => left.left < right.right && left.right > right.left && left.top < right.bottom && left.bottom > right.top)); const product = rects.find(rect => rect.label === "제품부터 설명"); const evidence = rects.find(rect => rect.label === "근거부터 설명"); return {within:rects.every(rect => rect.left >= -1 && rect.right <= innerWidth + 1 && rect.top >= -1 && rect.bottom <= innerHeight + 1), overlap, product, evidence, rects}; })()');
  assert('presenter controls stay bounded and non-overlapping', presenterControlLayout.within && !presenterControlLayout.overlap && presenterControlLayout.product?.width > 0 && presenterControlLayout.product?.height > 0 && presenterControlLayout.evidence?.width > 0 && presenterControlLayout.evidence?.height > 0, JSON.stringify(presenterControlLayout));
  if (viewportWidth <= 760) {
    const presenterRows = await evaluate('(() => { const buttons = [...document.querySelectorAll(".story--presentation .story-controls button")]; const rows = [...new Set(buttons.map(button => Math.round(button.getBoundingClientRect().top)))]; return {rows:rows.length,buttons:buttons.length}; })()');
    assert('presenter mobile controls form readable rows', presenterRows.rows >= 3 && presenterRows.buttons >= 7, JSON.stringify(presenterRows));
  }
  ...
  await waitForPresentation('01 / 11');
  await evaluate('document.querySelector(".story-evidence-start")?.click()');
  await waitForPresentation('06 / 11');
  const inPageEvidenceShortcut = await evaluate('({presentation:!!document.querySelector(".story--presentation"),progress:document.querySelector(".story-controls span")?.innerText,url:location.href})');
  assert('presenter can jump to evidence without leaving the flow', inPageEvidenceShortcut.presentation && inPageEvidenceShortcut.progress === '06 / 11' && inPageEvidenceShortcut.url.includes('mode=presenter') && inPageEvidenceShortcut.url.includes('card=6'), JSON.stringify(inPageEvidenceShortcut));
  await evaluate('document.querySelector(".story-restart-button")?.click()');
  await waitForPresentation('01 / 11');
  ...
  const researchReadableLayout = await evaluate('(() => { const values = selector => { const rect=document.querySelector(selector)?.getBoundingClientRect(); return rect ? {top:rect.top,bottom:rect.bottom,height:rect.height} : null; }; const boundary=values(".info-panel__boundary"); const source=values(".info-panel__source"); const flowNote=values(".info-panel__flow-note"); const actions=values(".info-panel__actions"); const overlaps=(first, second) => !!first && !!second && first.bottom > second.top + 1 && first.top < second.bottom - 1; return {boundary,source,flowNote,actions,boundaryCovered:overlaps(boundary,actions),sourceCovered:overlaps(source,actions),flowNoteCovered:overlaps(flowNote,actions),wrapperPosition:getComputedStyle(document.querySelector(".info-panel__actions"))?.position||"",nextPosition:getComputedStyle(document.querySelector(".info-panel__next"))?.position||"",backgroundInert:[...document.querySelectorAll("#story > :not(.info-layer)")].every(element=>element.hasAttribute("inert"))}; })()');
  assert('research evidence stays clear of sticky actions', !researchReadableLayout.boundaryCovered && !researchReadableLayout.sourceCovered && !researchReadableLayout.flowNoteCovered && researchReadableLayout.wrapperPosition === 'sticky' && researchReadableLayout.backgroundInert, JSON.stringify(researchReadableLayout));
  const researchActionOrder = await evaluate('([...document.querySelectorAll(".info-panel > .info-panel__actions, .info-panel > .info-panel__external-choice")].map(element => element.className).join("|"))');
  assert('in-page next action precedes external source', researchActionOrder === 'info-panel__actions|info-panel__external-choice', researchActionOrder);
  ...
  await wait(350);
  const productCard = await evaluate('({dialog:!!document.querySelector("[role=dialog]"),progress:document.querySelector(".story-controls span")?.innerText,focus:document.activeElement?.innerText||"",focusId:document.activeElement?.id||""})');
  assert('next card continues the story', !productCard.dialog && productCard.progress === '07 / 11' && productCard.focus.includes('제품 구성') && productCard.focusId === 'slide-product', JSON.stringify(productCard));
  ...
  await wait(180);
  const product = await evaluate('({title:document.querySelector("[role=dialog] h2")?.innerText||"",facts:document.querySelector(".product-facts")?.innerText||"",status:document.querySelector(".info-panel__status")?.innerText||"",customerCopy:!!document.querySelector(".info-panel__customer-copy")})');
  assert('product opens in the same dialog flow', product.title.includes('제품 정보') && product.facts.includes('셀핀다 가바 1500') && product.status.includes('최종 제품 사실로 확정하지 않습니다') && !product.customerCopy, JSON.stringify(product));
  ...
  const productPanelLayout = await evaluate('(() => { const panel=document.querySelector(".info-panel")?.getBoundingClientRect(); const actions=document.querySelector(".info-panel__actions"); const actionRect=actions?.getBoundingClientRect(); const next=document.querySelector(".info-panel__next")?.getBoundingClientRect(); return {viewportHeight:innerHeight,panelTop:panel?.top??-1,panelBottom:panel?.bottom??-1,actionsBottom:actionRect?.bottom??-1,nextHeight:next?.height??0,position:actions?getComputedStyle(actions).position:""}; })()');
  assert('product panel keeps sticky next action inside viewport', productPanelLayout.panelTop >= 0 && productPanelLayout.panelBottom <= productPanelLayout.viewportHeight + 1 && productPanelLayout.actionsBottom <= productPanelLayout.panelBottom + 1 && productPanelLayout.nextHeight > 0 && productPanelLayout.position === 'sticky', JSON.stringify(productPanelLayout));
  ...
  await evaluate('document.querySelector("#story-card-research .card-link")?.click()');
  await wait(180);
  const presenterPanelCustomerCopy = await evaluate('(() => { const button=document.querySelector(".info-panel__customer-copy"); const panel=document.querySelector(".info-panel")?.getBoundingClientRect(); return {count:document.querySelectorAll(".info-panel__customer-copy").length,visible:!!button && button.getBoundingClientRect().height > 0,within:!!panel && !!button && button.getBoundingClientRect().left >= panel.left - 1 && button.getBoundingClientRect().right <= panel.right + 1,presentation:!!document.querySelector(".story--presentation"),progress:document.querySelector(".story-controls span")?.innerText||""}; })()');
  assert('presenter panel exposes one in-page customer link action', presenterPanelCustomerCopy.count === 1 && presenterPanelCustomerCopy.visible && presenterPanelCustomerCopy.within && presenterPanelCustomerCopy.presentation && presenterPanelCustomerCopy.progress === '06 / 11', JSON.stringify(presenterPanelCustomerCopy));
  const panelCustomerCopyMocked = await evaluate('(() => { try { window.__qaPanelCopied = []; Object.defineProperty(navigator, "clipboard", {configurable:true, value:{writeText: async text => { window.__qaPanelCopied.push(text); }}}); return true; } catch { return false; } })()');
  if (panelCustomerCopyMocked) {
    await evaluate('document.querySelector(".info-panel__customer-copy")?.click()');
    await wait(120);
    const panelCustomerLink = await evaluate('(() => { const raw=window.__qaPanelCopied?.[0]||""; try { const url=new URL(raw); return {raw,card:url.searchParams.get("card"),mode:url.searchParams.get("mode"),presenter:url.searchParams.get("presenter"),hash:url.hash,origin:url.origin,path:url.pathname,currentOrigin:location.origin,currentPath:location.pathname,dialog:!!document.querySelector(".info-panel"),presentation:!!document.querySelector(".story--presentation"),progress:document.querySelector(".story-controls span")?.innerText||"",message:document.querySelector(".info-panel__customer-message")?.innerText||"",focusInside:!!document.activeElement?.closest(".info-panel")}; } catch { return {raw,card:"",mode:"",presenter:"",hash:"",origin:"",path:"",currentOrigin:location.origin,currentPath:location.pathname,dialog:!!document.querySelector(".info-panel"),presentation:!!document.querySelector(".story--presentation"),progress:document.querySelector(".story-controls span")?.innerText||"",message:document.querySelector(".info-panel__customer-message")?.innerText||"",focusInside:!!document.activeElement?.closest(".info-panel")}; } })()');
    assert('panel customer link stays in the story and strips presenter mode', panelCustomerLink.card === '6' && !panelCustomerLink.mode && !panelCustomerLink.presenter && panelCustomerLink.hash === '#story' && panelCustomerLink.origin === panelCustomerLink.currentOrigin && panelCustomerLink.path === panelCustomerLink.currentPath && panelCustomerLink.dialog && panelCustomerLink.presentation && panelCustomerLink.progress === '06 / 11' && panelCustomerLink.message.includes('고객용 카드 링크를 복사했습니다') && panelCustomerLink.focusInside, JSON.stringify(panelCustomerLink));
  }
  await evaluate('document.querySelector(".info-panel__next")?.click()');
  await wait(350);
  const presenterPanelNext = await evaluate('({dialog:!!document.querySelector(".info-panel"),progress:document.querySelector(".story-controls span")?.innerText,focus:document.activeElement?.innerText||"",focusId:document.activeElement?.id||""})');
  assert('presenter panel next keeps focus in the story', !presenterPanelNext.dialog && presenterPanelNext.progress === '07 / 11' && presenterPanelNext.focusId === 'slide-product' && presenterPanelNext.focus.includes('제품 구성'), JSON.stringify(presenterPanelNext));
